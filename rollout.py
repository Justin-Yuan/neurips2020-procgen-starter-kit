...
def rollout(agent,
    ...
    # NOTE: prep for video saving 
    img_list = []
    prefix = agent.config.get("run", "exp")    # get trainer name
    timestamp = datetime.now().strftime("%b-%d-%H-%M")  # till minute %Y-%b-%d-%H-%M
    log_dir = os.path.join(
        "videos", 
        "{}_{}_{}".format(prefix, env_name, timestamp)
    )
    ...
    while keep_going(steps, num_steps, episodes, num_episodes):
        ...
        if done:
            episodes += 1
            # NOTE: save rollout videos 
            img_list.append(next_obs)
            log_video(log_dir, str(episodes)+".gif", img_list)
